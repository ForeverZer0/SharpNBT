...
public class TagBuilder
{
    ...
    private readonly Stack<Tag> tree;
    ...
    /// </summary>
    /// <param name="name">
    /// The name of the top-level <see cref="CompoundTag"/>, or <see langword="null"/> to indicate it is unnamed.
    /// </param>
    ...
    public TagBuilder AddByte(string? name, int value) => AddByte(name, (byte)(value & 0xFF));
    ...
    public TagBuilder AddByte(int value) => AddByte(null, (byte)(value & 0xFF));
    ...
    /// <inheritdoc cref="AddShort(string,short)"/>
    public TagBuilder AddShort(string? name, int value) => AddShort(name, (short)(value & 0xFFFF));
    ...
    /// <inheritdoc cref="AddShort(short)"/>
    public TagBuilder AddShort(int value) => AddShort((short)(value & 0xFFFF));
    ...
    {
        var top = tree.Peek();
        if (top is CompoundTag compound)
            compound.Add(tag);
        else if (top is ListTag list)
            list.Add(tag);
        else
            throw new InvalidOperationException($"Invalid state, expected {nameof(CompoundTag)} or {nameof(ListTag)}.");
        return this;
    }
    ...
    public TagBuilder BeginList(string? name = null)
    {
    ...
    {
        var top = tree.Peek();
        if (top is ListTag || (tree.Count > 1 && top is CompoundTag))
            tree.Pop();
        return this;
    }
    ...
    public Context NewCompound(string? name = null)
    {
        BeginCompound(name);
    ...
    public Context NewList(string? name = null)
    {
        BeginList(name);
    ...
    /// </summary>
    /// <remarks>
    /// Implements <see cref="IDisposable"/> to that each node can used with <c>using</c> statements for easily
    /// distinguishable scope. This additionally provides a mechanism for the builder to automatically keep its
    /// stack in sync.
    /// </remarks>
    [PublicAPI]
    public class Context : IDisposable
    {
        ...
        internal Context(Tag tag, CloseHandler handler)
        {
            Tag = tag;
